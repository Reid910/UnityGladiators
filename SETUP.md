...
- **Swap-target feedback added**: `ItemPickup.nameLabel` now shows `[E] <item
  ...
  it visually pops if several pickups are near each other — **when wiring
  the `ItemPickup` prefab** (currently only exists on the
  `chore/m1-m5-setup-and-fixes` branch, not this one), assign `Visual
  Transform` to the mesh's own child transform, *not* the prefab root, so
  the trigger collider doesn't grow along with it. Leaving it unassigned is
  safe — the label-only feedback still works, it just won't scale.
